...
class BillionaireService:
    ...
    def aggiungere_miliardario(self, corpo_richiesta):
        ...
        if not esito_validazione:
            return {"codice": 400, "messaggio": "Validazione errata"}, 400

        nome_industria = corpo_richiesta['industrie']['industries']

        record_industria = self.repository.recupero_singolo(
            "SELECT id FROM billionaires_sistemato_industrie WHERE industries = %s", (nome_industria,)
        )
        ...
        if record_industria:
            ...
            inserimento = self.repository.manipolazione(
                "INSERT INTO billionaires_sistemato_industrie (industries) VALUES (%s)", (nome_industria,)
            )
            if isinstance(inserimento, str):
    ...
    def eliminare_miliardario(self, id):
        ...
        if not altri:
            ...
            self.repository.manipolazione(
                "DELETE FROM billionaires_sistemato_industrie WHERE id = %s", (id_industria,)
            )
